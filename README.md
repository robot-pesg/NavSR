...
<div align=center><img src="https://github.com/robot-pesg/NavSR/blob/main/demo/visualization_demo.gif" width="80%" height="80%" /></div>
...
<details>  
<summary><b>Synchronization and Calibration</b></summary>
Time synchronization in our multisensor robotic system is achieved through a custom hardware Trigger and Timing board and a PTP network. The board, built upon an STM32 MCU, generates synchronized pulses at 1 Hz, 40 Hz, and 400 Hz.
For camera intrinsic and extrinsic calibration, we employ the mature <a href="https://www.mathworks.com/help/vision/camera-calibration.html" target="_blank" rel="noopener">MATLAB calibration toolbox</a>, achieving an average reprojection error below 0.1 pixel. For IMU intrinsic calibration, we use the <a href="https://github.com/ori-drs/allan_variance_ros" target="_blank" rel="noopener">allan_variance_ros</a> toolbox developed by the Oxford Robotics Institute, estimating the noise densities and random walks of the accelerometers and gyroscopes from 20 hours of static data. The camera-IMU extrinsics are estimated using <a href="https://github.com/ethz-asl/kalibr" target="_blank" rel="noopener">Kalibr toolbox</a>, resulting in a reprojection error of less than 0.5 pixel. For camera-LiDAR calibration, we adopt a <a href="https://github.com/wds320/BotanicGarden/tree/main/calibration_tools" target="_blank" rel="noopener">self-developed toolbox</a> that utilizes 3D checking boards and pointto-plane ICP registration to estimate the extrinsics between the left RGB camera and the Livox LiDAR.
</details>
